...
//
// Two drawings: left-to-right, and top-to-bottom sized to the ~250px a narrow
// card leaves so the text renders at its real size instead of being scaled
// down. `responsive` switches at `md` (the home card, which sits next to the
// Azure one from there up); `vertical` is for cards that are narrow at every
// width, like the Apps grid.
export function Api0FlowDiagram({ layout = 'responsive' }: { layout?: 'responsive' | 'vertical' }) {
  ...
  return (
    <>
      ...
      <div className={`${layout === 'responsive' ? 'md:hidden ' : ''}w-full max-w-[250px] mx-auto`}>
        <svg
          viewBox="-2 -2 254 206"
          width="100%"
  ...
  return (
    <>
      ...
      <g fill="#ffffff" textAnchor="middle">
        <text x={cx} y={y + 24} className="text-[14px] font-bold">
          API0.AI
        </text>
        <text x={cx} y={y + 42} className="text-[11px] font-semibold">
          {t('api0_diagram_gateway')}
        </text>
      </g>
    </>
  );
